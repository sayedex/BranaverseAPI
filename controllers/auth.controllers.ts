...
export const login = catchAsyncErrors(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!isCorrect) return next(createError(400, "invalid anything"));
...
export const loginotp = catchAsyncErrors(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!existEmail) return next(createError(400, "invalid anything"));

    const updateData: any = await userotps.findByIdAndUpdate(
      { _id: existEmail._id },
      {
        otp: OTP,
        expire,
...
export const OTP = catchAsyncErrors(
  async (req: Request, res: Response, next: NextFunction) => {
    console.log("call");
...
//User password reset
export const passwordchange = catchAsyncErrors(
  async (req: any, res: Response, next: NextFunction) => {
    const { oldpassword, password, confirmPassword } = req.body;
    if (!confirmPassword || !password)
      return next(createError(404, "invalid anything"));
    ...
    if (!isPasswordMatched) return next(createError(401, "invalid password"));
...
//User password reset
export const validOTP = catchAsyncErrors(
  async (req: Request, res: Response, next: NextFunction) => {
...
export const getUserDetails = catchAsyncErrors(
  async (req: any, res: Response, next: NextFunction) => {
    ...
    if (user && user.refId && user.refId.length != 0) {
      res.status(200).json({
        success: true,
        user,
        totalRefferReward: user.refId.length * REFFER_REWARD!,
      });
    } else {
      res.status(200).json({
        success: true,
